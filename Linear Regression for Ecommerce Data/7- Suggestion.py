...
# Step 2: Provide a suggestion to the company based on the coefficients of the model
# The coefficients represent the impact of each feature on the target variable (Yearly Amount Spent)
coefficients = pd.Series(linear_model.coef_, index=X.columns)
print("\nStep 6: Coefficients of the Model")
print(coefficients)

# "Time on App" feature has a larger positive coefficient, it indicates that increasing that feature can lead to higher Yearly Amount Spent
# "Time on Website" feature has a larger negative coefficient, it indicates that decreasing that feature can lead to higher Yearly Amount Spent
